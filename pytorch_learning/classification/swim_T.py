...
def window_partition(x, window_size: int):
    B, H, W, C = x.shape
    x = x.view(B, H // window_size, window_size, B // window_size, window_size, C)
...
class PatchEmbed(nn.Module):
    def __init__(self, path_size=4, in_c=3, embed_dim=96, norm_layer=None):
        super().__init__()
        path_size = (path_size, path_size)
        self.patch_size = path_size
        self.in_chans = in_c
        self.embed_dim = embed_dim
        self.proj = nn.Conv2d(in_c, embed_dim, kernel_size=path_size, stride=path_size)
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x, H, W):
        attn_mask = self.creat_mask(x, H, W)
