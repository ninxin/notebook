...
def build_target(target: torch.Tensor, num_classes: int = 2, ignore_index: int = -100):
    ...
    if ignore_index >= 0:
        ignore_mask = torch.eq(target, ignore_index)        # 相同位置的元素相等时返回True
    ...
    return dice_target.permute(0, 3, 1, 2)      # B,H,W,C -> B,C,H,W
...
def dice_coeff(x: torch.Tensor, target: torch.Tensor, ignore_index: int = -100, epsilon=1e-6):
    ...
    for i in range(batch_size):
        ...
        if ignore_index >= 0:
            ...
            roi_mask = torch.ne(t_i, ignore_index)      # 相同位置元素不同时为True
            x_i = x_i[roi_mask]
            t_i = t_i[roi_mask]
