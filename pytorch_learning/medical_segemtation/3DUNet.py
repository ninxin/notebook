...
class Modified3DUNet(nn.Module):
    ...
    def forward(self, x):
        #  Level 1 context pathway
        out = self.conv3d_c1_1(x)           # _, 8
        residual_1 = out
        out = self.lrelu(out)
        out = self.conv3d_c1_2(out)         # 8, 8
        out = self.dropout3d(out)
        out = self.lrelu_conv_c1(out)       # 8, 8
        # Element Wise Summation
        out += residual_1
        context_1 = self.lrelu(out)
        out = self.inorm3d_c1(out)          # 8, 8
        out = self.lrelu(out)

        # Level 2 context pathway
        out = self.conv3d_c2(out)           # 8， 16
        residual_2 = out
        out = self.norm_lrelu_conv_c2(out)          # 16, 16
        out = self.dropout3d(out)
        out = self.norm_lrelu_conv_c2(out)          # 16, 16
        out += residual_2
        ...
        # Level 3 context pathway
        out = self.conv3d_c3(out)           # 16， 32
        residual_3 = out
        out = self.norm_lrelu_conv_c3(out)          # 32, 32
        out = self.dropout3d(out)
        out = self.norm_lrelu_conv_c3(out)          # 32, 32
        out += residual_3
        ...
        # Level 4 context pathway
        out = self.conv3d_c4(out)           # 32， 64
        residual_4 = out
        out = self.norm_lrelu_conv_c4(out)          # 64, 64
        out = self.dropout3d(out)
        out = self.norm_lrelu_conv_c4(out)          # 64, 64
        out += residual_4
        ...
        # Level 5
        out = self.conv3d_c5(out)           # 64， 128
        residual_5 = out
        out = self.norm_lrelu_conv_c5(out)          # 128, 128
        out = self.dropout3d(out)
        out = self.norm_lrelu_conv_c5(out)          # 128, 128
        out += residual_5
        out = self.norm_lrelu_upscale_conv_norm_lrelu_l0(out)       # 128, 64

        out = self.conv3d_l0(out)           # 64, 64
        out = self.inorm3d_l0(out)
        out = self.lrelu(out)

        # Level 1 localization pathway
        out = torch.cat([out, context_4], dim=1)
        out = self.conv_norm_lrelu_l1(out)          # 128, 128
        out = self.conv3d_l1(out)                   # 128, 64
        out = self.norm_lrelu_upscale_conv_norm_lrelu_l1(out)       # 64, 32

        # Level 2 localization pathway
        out = torch.cat([out, context_3], dim=1)
        out = self.conv_norm_lrelu_l2(out)          # 64, 64
        ds2 = out
        out = self.conv3d_l2(out)                   # 64, 32
        out = self.norm_lrelu_upscale_conv_norm_lrelu_l2(out)       # 32, 16

        # Level 3 localization pathway
        out = torch.cat([out, context_2], dim=1)
        out = self.conv_norm_lrelu_l3(out)          # 32, 32
        ds3 = out
        out = self.conv3d_l3(out)                   # 32, 16
        out = self.norm_lrelu_upscale_conv_norm_lrelu_l3(out)       # 16, 8

        # Level 4 localization pathway
        out = torch.cat([out, context_1], dim=1)
        out = self.conv_norm_lrelu_l4(out)          # 16, 16
        out_pred = self.conv3d_l4(out)              # 16, _

        ds2_1x1_conv = self.ds2_1x1_conv3d(ds2)         # 64, _
        ds1_ds2_sum_upscale = self.upsacle(ds2_1x1_conv)
        ds3_1x1_conv = self.ds3_1x1_conv3d(ds3)         # 32, _
