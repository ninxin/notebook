...
class ConvBnRelu2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, padding=1, dilation=1, stride=1, groups=1, is_bn=False,
        ...
        if BatchNorm:
            ...
            self.bn = nn.GroupNorm(out_channels, out_channels, eps=BN_EPS)
...
def diff_y(input, r):
    ...
    left = input[:, :, :, r:2 * r + 1]
    middle = input[:, :, :, 2 * r + 1:] - input[:, :, :, :-2 * r - 1]       # [2r+1:-1] - [0:-2r-1] 大小相同
    right = input[:, :, :, -1:] - input[:, :, :, -2 * r - 1:    -r - 1]
...
class AG_Net(nn.Module):
    ...
    def forward(self, x):
        ...
        x_2 = F.interpolate(x, size=(H // 2, W // 2), mode='bilinear')
        x_3 = F.interpolate(x, size=(H // 4, W // 4), mode='bilinear')
        x_4 = F.interpolate(x, size=(H // 8, W // 8), mode='bilinear')
        ...
        # print("FGshape", FG.shape)
        N, C, H, W = FG.size()
        FG_small = F.interpolate(FG, size=(H // 2, W // 2), mode='bilinear')
        # print("FG_smallshape", FG_small.shape)
        ...
        side_5 = F.interpolate(up5, size=(H, W), mode='bilinear')
        side_6 = F.interpolate(up6, size=(H, W), mode='bilinear')
        side_7 = F.interpolate(up7, size=(H, W), mode='bilinear')
        side_8 = F.interpolate(up8, size=(H, W), mode='bilinear')
        ...
        return {"out": ave_out}, {"out": side_5}, {"out": side_6}, {"out": side_7}, {"out": side_8}
