...
class DecoderBlock(nn.Module):
    def __init__(self, in_channels, n_filters):
        ...
        self.norm1 = nn.GroupNorm(in_channels // 4, in_channels // 4)
        self.relu1 = nonlinearity

        # self.deconv2 = nn.ConvTranspose2d(in_channels // 4, in_channels // 4, 3, stride=2, padding=1, output_padding=1)
        # self.norm2 = nn.BatchNorm2d(in_channels // 4)
        self.norm2 = nn.GroupNorm(in_channels // 4, in_channels // 4)
        self.relu2 = nonlinearity

        self.conv3 = nn.Conv2d(in_channels // 4, n_filters, 1)
        # self.norm3 = nn.BatchNorm2d(n_filters)
        self.norm3 = nn.GroupNorm(n_filters, n_filters)
...
class CE_Net_(nn.Module):
    def __init__(self, num_classes=2, num_channels=3):
        ...
        self.finalconv1 = nn.Conv2d(filters[0], 32, kernel_size=3, padding=1)
    ...
    def forward(self, x):
        ...
        # Decoder
        d4 = self.decoder4(e4, e3.shape[2:]) + e3
        d3 = self.decoder3(d4, e2.shape[2:]) + e2
        d2 = self.decoder2(d3, e1.shape[2:]) + e1
        d1 = self.decoder1(d2, x2.shape[2:])
...
class CE_Net_OCT(nn.Module):
    ...
    def forward(self, x):
        ...
        d3 = self.decoder3(d4, e2.shape[2:]) + e2
        d2 = self.decoder2(d3, e1.shape[2:]) + e1
        d1 = self.decoder1(d2, shape2)

        # out = self.finaldeconv1(d1)
        out = self.finalconv1(d1)
        out = F.interpolate(out, size=shape1, mode='bilinear')
        out = self.finalrelu1(out)
        out = self.finalconv2(out)
        out = self.finalrelu2(out)
        out = self.finalconv3(out)
