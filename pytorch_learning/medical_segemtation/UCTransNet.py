...
class Channel_Embeddings(nn.Module):
    ...
    def __init__(self, config, patchsize, img_size, in_channels):
        ...
        self.patch_embeddings = Conv2d(in_channels=in_channels,
                                       ...
                                       stride=patch_size)
        self.position_embeddings = nn.Parameter(torch.zeros(1, n_patches, in_channels))     # 将随机初始化的参数变成可训练的参数
        self.dropout = Dropout(config.transformer["embeddings_dropout_rate"])

    def forward(self, x):
        if x is None:
            return None
        x = self.patch_embeddings(x)  # (B, in_channels, n_patches^(1/2), n_patches^(1/2))
...
class Reconstruct(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.permute(0, 2, 1)
        x = x.contiguous().view(B, hidden, h, w)            # 相当于copy
